...
A fork of [this](https://github.com/geek-cookbook/traefik-forward-auth) with the following fix

* Add optional configuration to disable SSL cert verification, to be used ONLY for local development.
* Adds support to verify users by using the bearer token stored in this authorization header.  Traefik-forward-auth from  [this](https://github.com/geek-cookbook/traefik-forward-auth)
uses a session cookie to decide whether requests hitting Traefik has been authenticated.  While this works to allow SSO across multiple sites protected 
by Treafik, if the site itself is an API service where authorization token is expected on the method invocation level, there is a need to authenticate 
against that token.  This is the scenario where the API method is invoked from a non-browser client where setting a cookie is not possible. 
Long story short, the repo from [this](https://github.com/geek-cookbook/traefik-forward-auth) support SSO across sites when they are accessed via the browser, 
the addition in this repo adds support for hitting particular API methods with a token.
* Corrected mapping of keycloak user to user data struct.  This appears to be a bug with [this](https://github.com/geek-cookbook/traefik-forward-auth) where the 
user object retrieved had several fields being null (email, name.. etc) unless this mapping was corrected.
...
|-keycloak-host-port | string | host and port resolvable by Traefik Forward Auth container to control start up order      |
|-insecure   | bool | Whether or not to verify SSL certs. This should only be used for local development. If true, will set InsecureSkipVerify to true (default false) | 
|-token-header   | string | If provided, this offers an alternative method of verifying users by using the bearer token stored in this authorization header. (default "")
...
[MIT](https://github.com/black-cape/traefik-forward-auth/blob/master/LICENSE.md)